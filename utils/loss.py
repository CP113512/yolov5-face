    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            if model.nc > 1:  # cls loss (only if multiple classes)
                #--------------------------------------------
                t = torch.full_like(ps[:, 17:], cn, device=device)  # targets
                t[range(n), tcls[i]] = cp
                # --------------------------------------------
                lcls += BCEcls(ps[:, 17:], t)  # BCE
...
def build_targets(p, targets, model):
    ...
    # --------------------------------------------
    gain = torch.ones(19, device=targets.device)
    ...
    for i in range(det.nl):
        ...
        #landmarks 10
        # --------------------------------------------
        gain[6:18] = torch.tensor(p[i].shape)[[3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2]]  # xyxy gain
