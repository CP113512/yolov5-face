...
def non_max_suppression_face(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, labels=()):
    ...
    nc = prediction.shape[2] - 17  # number of classes
    ...
    t = time.time()
    # ___________________________________________
    output = [torch.zeros((0, 20), device=prediction.device)] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            l = labels[xi]
            # ___________________________________________
            v = torch.zeros((len(l), nc + 17), device=x.device)
            v[:, :4] = l[:, 1:5]  # box
            v[:, 4] = 1.0  # conf
            # ___________________________________________
            v[range(len(l)), l[:, 0].long() + 17] = 1.0  # cls
            x = torch.cat((x, v), 0)
        ...
        # Compute conf
        # ___________________________________________
        x[:, 17:] *= x[:, 4:5]  # conf = obj_conf * cls_conf
        ...
        if multi_label:
            # ___________________________________________
            i, j = (x[:, 17:] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i, j + 17, None], x[i, 5:17] ,j[:, None].float()), 1)
        else:  # best class only
            # ___________________________________________
            conf, j = x[:, 17:].max(1, keepdim=True)
            x = torch.cat((box, conf, x[:, 5:17], j.float()), 1)[conf.view(-1) > conf_thres]
